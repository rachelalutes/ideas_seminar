...
degrees = 0
sideLength = 10
startPoint = bob.point
startDirection = bob.direction
bob.right(180)
bob.forward(sideLength/2)
bob.direction = startDirection
for j in range(3):    
    for i in range(sides):
        joe = Turtle(bob.point,bob.heading)
        joe.ellipse(sideLength,sideLength/2,'edge',degrees)
        bob.forward(sideLength)
        bob.left(360/sides)
        degrees = degrees + (360/sides)
    degrees = 0
    sideLength = sideLength - 3
    rs.MoveObject(bob.point, rs.VectorCreate(startPoint, bob.point))
    newVector = rs.VectorScale(startDirection, sideLength/2)
    rs.MoveObject(bob.point,newVector)
    bob.direction = startDirection
